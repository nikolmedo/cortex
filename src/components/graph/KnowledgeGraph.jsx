...
import { CYAN, BG, TOP_BAR_H, BOTTOM_STRIP_H } from '../../constants/index.js';
import { computeLayout, floatOffset } from '../../layout/computeLayout.js';
import { loremflickrUrl } from '../../utils/image.js';
import { NodeDiv } from './NodeDiv.jsx';
import { CategoryNodeInner } from './CategoryNodeInner.jsx';
import { LoadingScene } from './LoadingScene.jsx';
import { Tooltip } from '../Tooltip.jsx';

export function KnowledgeGraph({ graphData, phase, hoveredCat, setHoveredCat, W, H }) {
  ...
  // Reset state when a new query result arrives
  useEffect(() => {
    if (!graphData) return;
    setFocusedCat(null);
  ...
  // ESC closes focused category
  useEffect(() => {
    if (focusedCat == null) return;
    const handler = e => { if (e.key === 'Escape') setFocusedCat(null); };
  ...
  // RAF floating loop — imperative DOM updates to avoid React re-renders on every frame
  useEffect(() => {
  ...
  // Sync SVG edge paths to current layout positions
  useEffect(() => {
    if (!baseLayout) return;
  ...
  return (
    ...
    >
      ...
      <svg style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', pointerEvents: 'none', zIndex: 1 }}>
        ...
        {/* Category → Fact (only for focused category) */}
        {facts.map((group, i) =>
          group.map((fact, j) => {
            const key   = `e-f-${i}-${j}`;
...
function FocusedLabel({ category, entityTitle }) {
