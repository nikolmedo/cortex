...
describe('sanitizeText', () => {
  ...
  it('neutralises angle brackets', () => {
    expect(sanitizeText('<img onerror=x>', 50)).toBe(`${LSAQ}img onerror=x${RSAQ}`);
  });
...
describe('hex helpers', () => {
  ...
  it('hexOr falls back for anything that is not a hex', () => {
    ...
    expect(hexOr(undefined, DEFAULT_PALETTE.accent)).toBe(DEFAULT_PALETTE.accent);
  });
});

describe('asEnum', () => {
  it('keeps a value that is in the allowlist', () => {
    expect(asEnum('mosaic', SCENE_LAYOUTS, 'focus')).toBe('mosaic');
    expect(asEnum('volatile', SCENE_MOODS, 'calm')).toBe('volatile');
  });

  it('replaces anything else with the fallback', () => {
    expect(asEnum('hologram', SCENE_LAYOUTS, 'focus')).toBe('focus');
    expect(asEnum('', SCENE_LAYOUTS, 'focus')).toBe('focus');
    expect(asEnum(42, SCENE_LAYOUTS, 'focus')).toBe('focus');
    expect(asEnum(null, SCENE_LAYOUTS, 'focus')).toBe('focus');
    expect(asEnum(undefined, SCENE_LAYOUTS, 'focus')).toBe('focus');
    expect(asEnum(['mosaic'], SCENE_LAYOUTS, 'focus')).toBe('focus');
  });
});
...
describe('sanitizeScene: structural failures', () => {
  it('throws when the payload is not an object', () => {
    expect(() => sanitizeScene('x')).toThrow(/not an object/);
    expect(() => sanitizeScene(null)).toThrow(/not an object/);
    expect(() => sanitizeScene([])).toThrow(/not an object/);
  });

  it('throws when the title is missing or empty', () => {
    expect(() => sanitizeScene({ modules: [] })).toThrow(/title is missing/);
    expect(() => sanitizeScene({ title: '   ', modules: [] })).toThrow(/title is missing/);
  });

  it('throws when modules is not an array', () => {
    expect(() => sanitizeScene({ title: 'T' })).toThrow(/modules is not an array/);
    expect(() => sanitizeScene({ title: 'T', modules: 'x' })).toThrow(/modules is not an array/);
  });

  it('accepts the minimal valid payload and fills in every default', () => {
    const scene = sanitizeScene({ title: 'T', modules: [] });
...
describe('sanitizeScene: hostile payload', () => {
  ...
  it('sanitises the title and coerces out-of-enum values to safe defaults', () => {
    ...
    expect(scene.presentation).toMatchObject({ layout: 'focus', mood: 'calm', motif: 'flow', density: 'balanced' });
  });
  ...
  it('normalises a code body and keeps its language', () => {
    expect(scene.modules[1]).toMatchObject({ kind: 'code', body: 'line1\nline2', value: 'python' });
  });
...
describe('sanitizePreface', () => {
  it('returns null for a non-object', () => {
    expect(sanitizePreface('x')).toBeNull();
    expect(sanitizePreface(null)).toBeNull();
    expect(sanitizePreface([])).toBeNull();
  });

  it('never throws on a hostile payload and coerces every field', () => {
    expect(sanitizePreface({
      intent: 'zzz',
      title: `P${NUL}`,
      layout: 'zzz',
      mood: 'zzz',
  ...
  it('derives the layout from the intent when it is absent', () => {
    expect(sanitizePreface({ intent: 'howto', title: 'P' })?.layout).toBe('sequence');
    expect(sanitizePreface({ intent: 'comparison', title: 'P' })?.layout).toBe('split');
    expect(sanitizePreface({})?.layout).toBe('focus');
  });
});
