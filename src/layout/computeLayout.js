import { CYAN, TOP_BAR_H, BOTTOM_STRIP_H } from '../constants/index.js';

export function computeLayout(graphData, W, H, focusedCat) {
  ...
  const categories = graphData.graph.map((cat, i) => {
  ...
  const facts = graphData.graph.map((cat, i) => {
    if (isFocused && i === focusedCat) {
...
export function floatOffset(k, t) {
  const TAU = 2 * Math.PI;
