...
import { CYAN, BG, TOP_BAR_H } from '../../../infrastructure/constants';
...
export function LoadingScene({ W, H }: LoadingSceneProps) {
  ...
  const r = Math.min(W, availH) * 0.34;
  ...
  return (
    <>
      <svg style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', pointerEvents: 'none', zIndex: 1 }}>
        ...
        {SWEEP_ARMS.map(({ dur, offset, opacity, width }, i) => (
          <g key={i} transform={`translate(${cx}, ${cy})`}>
            <g
              ...
              style={{ animationDuration: dur, transform: `rotate(${offset}deg)`, transformOrigin: '0 0' }}
            >
              <path
        ...
        {([0, 45, 90, 135, 180, 225, 270, 315] as const).map(deg => {
          ...
          return (
            <line
              key={deg}
              x1={cx + (r - len) * Math.cos(rad)}
              y1={cy + (r - len) * Math.sin(rad)}
              x2={cx + r * Math.cos(rad)}
              y2={cy + r * Math.sin(rad)}
              stroke={`${CYAN}${major ? '88' : '44'}`}
            ...
            style={
              {
                ...
                '--drift': `translate(${p.driftX}px, ${p.driftY}px)`,
          ...
          {(['', '0.7s', '1.4s'] as const).map((delay, i) => (
            <div
              ...
              style={{
                ...
                height: 80,
                borderRadius: '50%',
                border: `1px solid ${CYAN}${i === 0 ? '' : i === 1 ? '88' : '55'}`,
      ...
      >
        ...
        <span style={{ color: CYAN }}>{t(LOADING_STEPS[statusIdx])}</span>
      ...
      >
        {LOADING_STEPS.slice(0, statusIdx + 1).map((step, i) => (
